...
function ciniki_customers_wng_changeTempPassword($ciniki, $tnid, $request, $email, $temppassword, $newpassword) {
    ...
    if( strlen($newpassword) < 8 ) {
        ciniki_customers_wng_logAdd($ciniki, $tnid, $request, 50, 'Reset Password', 0, $email, 'ciniki.customers.193', 'New password must be 8 characters long');
        error_log("WEB [" . $ciniki['tenant']['name'] . "]: changeTempPassword $email fail (730)");
    ...
    if( $rc['stat'] != 'ok' ) {
        ...
        error_log("WEB [" . $ciniki['tenant']['name'] . "]: changeTempPassword $email fail (" . $rc['err']['code'] . ")");
        return $rc;
    }
    if( !isset($rc['rows']) || count($rc['rows']) == 0 ) {
        ciniki_customers_wng_logAdd($ciniki, $tnid, $request, 50, 'Reset Password', 0, $email, 'ciniki.customers.194', 'Temp password incorrect');
        error_log("WEB [" . $ciniki['tenant']['name'] . "]: changeTempPassword $email fail (731)");
    ...
    //
//  if( $rc['num_rows'] != 1 ) {
//      error_log("WEB [" . $ciniki['tenant']['name'] . "]: changeTempPassword $email fail (732)");
...
//  }
    ...
    foreach($users as $user) {
        ...
        if( $rc['stat'] != 'ok' ) {
            ...
            ciniki_customers_wng_logAdd($ciniki, $tnid, $request, 50, 'Reset Password', 0, $email, 'ciniki.customers.196', 'Internal error');
            error_log("WEB [" . $ciniki['tenant']['name'] . "]: changeTempPassword $email fail (733)");
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.customers.196', 'msg'=>'Unable to update password.'));
        }

        if( $rc['num_affected_rows'] < 1 ) {
            ciniki_core_dbTransactionRollback($ciniki, 'ciniki.customers');
            ciniki_customers_wng_logAdd($ciniki, $tnid, $request, 50, 'Reset Password', 0, $email, 'ciniki.customers.197', 'Unable to change password');
            error_log("WEB [" . $ciniki['tenant']['name'] . "]: changeTempPassword $email fail (734)");
    ...
    if( $rc['stat'] != 'ok' ) {
        ciniki_customers_wng_logAdd($ciniki, $tnid, $settings, 50, 'Reset Password', 0, $email, 'ciniki.customers.198', 'Unable to commit changes');
        error_log("WEB [" . $ciniki['tenant']['name'] . "]: changeTempPassword $email fail (735)");
    ...
    error_log("WEB [" . $ciniki['tenant']['name'] . "]: changeTempPassword $email success");
