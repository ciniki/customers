...
function ciniki_customers_sync_customerGet($ciniki, $sync, $business_id, $args) {
	...
	if( !isset($args['customer']) || $args['customer'] == '' ) {
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'263', 'msg'=>'No type specified'));
	}
	...
	$strsql = "SELECT ciniki_customers.uuid AS customer_uuid, "
		...
		. "AND ciniki_customers.uuid = '" . ciniki_core_dbQuote($ciniki, $args['customer']) . "' "
		. "ORDER BY log_date "
		. "";
	...
	if( !isset($rc['customers'][$args['customer']]) ) {
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'164', 'msg'=>'Customer does not exist'));
	}
	$customer = $rc['customers'][$args['customer']];

	//
	// Get the customer email information
	//
	$strsql = "SELECT ciniki_customer_emails.uuid AS email_uuid, ciniki_customer_emails.email, ciniki_customer_emails.password, "
...
//		. "ciniki_customer_history.new_value, "
	...
	$rc = ciniki_core_dbHashQueryIDTree($ciniki, $strsql, 'ciniki.customers', array(
		array('container'=>'emails', 'fname'=>'email_uuid', 
			'fields'=>array('uuid'=>'email_uuid', 'email', 'password', 'temp_password', 'temp_password_date', 'flags',
				'date_added', 'last_updated')),
		array('container'=>'history', 'fname'=>'history_uuid', 
			'fields'=>array('user'=>'user_uuid', 'session', 
				'action', 'table_field', 'new_value', 'log_date')),
	...
	$rc = ciniki_core_dbHashQueryIDTree($ciniki, $strsql, 'ciniki.customers', array(
		array('container'=>'addresses', 'fname'=>'address_uuid', 
			'fields'=>array('flags', 'address1', 'address2', 'city', 'province', 'postal', 'country',
				'date_added', 'last_updated')),
		array('container'=>'history', 'fname'=>'history_uuid', 
			'fields'=>array('user'=>'user_uuid', 'session', 
				'action', 'table_field', 'new_value', 'log_date')),
