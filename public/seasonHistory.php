...
// api_key:
// auth_token:
// business_id:         The ID of the business to get the season history for.
...
function ciniki_customers_seasonHistory($ciniki) {
    ...
    $rc = ciniki_customers_checkAccess($ciniki, $args['business_id'], 'ciniki.customers.seasonHistory', 0);
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    return ciniki_core_dbGetModuleHistory($ciniki, 'ciniki.customers', 'ciniki_customer_history', 
        $args['business_id'], 'ciniki_customer_seasons', $args['season_id'], $args['field']);
}
?>
