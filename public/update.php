...
function ciniki_customers_update($ciniki) {
	...
	//
	$strsql = "UPDATE customers SET last_updated = UTC_TIMESTAMP()";
	...
	foreach($changelog_fields as $field) {
		if( isset($args[$field]) ) {
			...
			$rc = ciniki_core_dbAddChangeLog($ciniki, 'customers', $args['business_id'], 
				'customers', $args['customer_id'], $field, $args[$field]);
		}
	}
	...
	if( !isset($rc['num_affected_rows']) || $rc['num_affected_rows'] != 1 ) {
		ciniki_core_dbTransactionRollback($ciniki, 'customers');
		return array('stat'=>'fail', 'err'=>array('code'=>'371', 'msg'=>'Unable to add customer'));
	}
