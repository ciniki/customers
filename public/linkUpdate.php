...
function ciniki_customers_linkUpdate(&$ciniki) {
    ...
    $rc = ciniki_customers_checkAccess($ciniki, $args['tnid'], 'ciniki.customers.linkUpdate', 0); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.customers.link', $args['link_id'], $args, 0x07);
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
