...
// -------
//
function ciniki_customers_customerDelete(&$ciniki, $tnid, $customer_id) {

    //
    // get the active modules for the tenant
    //
    ciniki_core_loadMethod($ciniki, 'ciniki', 'tenants', 'private', 'getActiveModules');
    $rc = ciniki_tenants_getActiveModules($ciniki, $tnid); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    $modules = $rc['modules'];
    ...
    $strsql = "SELECT COUNT(*) as num_children "
        ...
        . "WHERE tnid = '" . ciniki_core_dbQuote($ciniki, $tnid) . "' "
        . "AND parent_id = '" . ciniki_core_dbQuote($ciniki, $customer_id) . "' "
        . "";
    ...
    $rc = ciniki_core_objectCheckUsed($ciniki, $tnid, 'ciniki.customers.customer', $customer_id);
    if( $rc['stat'] != 'ok' ) {
    ...
    if( isset($modules['ciniki.subscriptions']) ) {
        ...
        if( isset($rc['rows']) ) {
            ...
            foreach($subscriptions as $i => $row) {
                $rc = ciniki_core_objectDelete($ciniki, $tnid, 'ciniki.subscriptions.customer',
    ...
    if( isset($rc['rows']) ) {
        ...
        foreach($items as $item) {
            $rc = ciniki_core_objectDelete($ciniki, $tnid, 'ciniki.customers.phone',
    ...
    if( isset($rc['rows']) ) {
        $items = $rc['rows'];
        foreach($items as $item) {
            $rc = ciniki_core_objectDelete($ciniki, $tnid, 'ciniki.customers.address',
                $item['id'], $item['uuid'], 0x04);
            if( $rc['stat'] != 'ok' ) {
                ciniki_core_dbTransactionRollback($ciniki, 'ciniki.customers');
                return $rc;
    ...
    if( isset($rc['rows']) ) {
        $items = $rc['rows'];
        foreach($items as $item) {
            $rc = ciniki_core_objectDelete($ciniki, $tnid, 'ciniki.customers.email',
                $item['id'], $item['uuid'], 0x04);
            if( $rc['stat'] != 'ok' ) {
                ciniki_core_dbTransactionRollback($ciniki, 'ciniki.customers');
                return $rc;
    ...
    if( isset($rc['rows']) ) {
        $items = $rc['rows'];
        foreach($items as $item) {
            $rc = ciniki_core_objectDelete($ciniki, $tnid, 'ciniki.customers.link',
                $item['id'], $item['uuid'], 0x04);
            if( $rc['stat'] != 'ok' ) {
                ciniki_core_dbTransactionRollback($ciniki, 'ciniki.customers');
                return $rc;
    ...
    // Delete the customer
    //
    $rc = ciniki_core_objectDelete($ciniki, $tnid, 'ciniki.customers.customer', $customer_id, $uuid, 0x04);
    if( $rc['stat'] != 'ok' ) {
        ciniki_core_dbTransactionRollback($ciniki, 'ciniki.customers');
