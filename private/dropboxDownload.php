...
function ciniki_customers_dropboxDownload(&$ciniki, $tnid) {
    ...
    if( !ciniki_core_checkModuleFlags($ciniki, 'ciniki.customers', 0x0800000000) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.customers.42', 'msg'=>'Dropbox integration not enabled'));
    }
    ...
    if( !isset($rc['settings']['dropbox-customerprofiles']) || $rc['settings']['dropbox-customerprofiles'] == '') {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.customers.43', 'msg'=>'Dropbox customers not setup.'));
    }
    ...
    //
    ciniki_core_loadMethod($ciniki, 'ciniki', 'core', 'private', 'dbDetailsQueryDash');
    $rc = ciniki_core_dbDetailsQueryDash($ciniki, 'ciniki_tenant_details', 'tnid', $tnid, 'ciniki.tenants', 'settings', 'apis');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    if( !isset($rc['settings']['apis-dropbox-access-token']) || $rc['settings']['apis-dropbox-access-token'] == '' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.customers.44', 'msg'=>'Dropbox not configured.'));
    }
    ...
    foreach($updates as $customer_eid => $customer) {
        ...
        foreach($customer as $field => $details) {
            ...
            elseif( $field == 'images' ) {
                $rc = ciniki_customers_dropboxDownloadImages($ciniki, $tnid, $client, $ciniki_customer, $details);
                if( $rc['stat'] != 'ok' ) {
                    ciniki_core_dbTransactionRollback($ciniki, 'ciniki.customers');
                    return $rc;
                }
        ...
        if( count($update_args) > 0 ) {
            $rc = ciniki_core_objectUpdate($ciniki, $tnid, 'ciniki.customers.customer', $customer_id, $update_args, 0x04);
            if( $rc['stat'] != 'ok' ) {
                ciniki_core_dbTransactionRollback($ciniki, 'ciniki.customers');
                return $rc;
            }
        ...
        ciniki_core_hookExec($ciniki, $tnid, 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.customers.customer', 'object_id'=>$customer_id));
        ciniki_core_hookExec($ciniki, $tnid, 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.customers.members', 'object_id'=>$customer_id));
        ciniki_core_hookExec($ciniki, $tnid, 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.customers.dealers', 'object_id'=>$customer_id));
        ciniki_core_hookExec($ciniki, $tnid, 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.customers.distributors', 'object_id'=>$customer_id));
    ...
    ciniki_core_dbAddModuleHistory($ciniki, 'ciniki.customers', 'ciniki_customer_history', $tnid, 
