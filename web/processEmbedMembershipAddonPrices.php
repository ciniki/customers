...
function ciniki_customers_web_processEmbedMembershipAddonPrices(&$ciniki, $settings, $tnid, $args) {
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.customers.503', 'msg'=>'Unable to load products', 'err'=>$rc['err']));
    }
    ...
    if( isset($ciniki['session']['customer']['id']) && $ciniki['session']['customer']['id'] > 0 ) {
        ...
        if( $rc['stat'] != 'ok' ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.customers.502', 'msg'=>'Unable to load purchases', 'err'=>$rc['err']));
        }
